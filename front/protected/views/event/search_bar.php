<div id="search_bar">
    <form action="" method="get" name="searchform" id="searchform" class="form-search clearfix">
        <?php foreach ($query_string as $k => $v): ?>
            <?php if ($k == "country" || $k == "cate") continue; ?>
        ...
        <select class="search-location pull-left" name="country">
            <option value="0">--Choose a country--</option>
            <?php foreach($countries as $c): ?>
            <option <?php if(isset($_GET['country']) && $_GET['country'] == $c['id']) echo 'selected' ?> value="<?php echo $c['id'] ?>"><?php echo $c['title'] ?></option>
            <?php endforeach; ?>
        </select>
