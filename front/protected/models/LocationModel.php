...
class LocationModel extends CFormModel {
    ...
    public function gets($args,$page = 1,$ppp = 10) {
        ...
        if (isset($args['s']) && $args['s'] != "") {
            $custom.= " AND el.title like :title";
            $params[] = array('name' => ':title', 'value' => "%$args[s]%", 'type' => PDO::PARAM_STR);
        }

        if (isset($args['deleted'])) {
            $custom.= " AND el.deleted = :deleted";
            $params[] = array('name' => ':deleted', 'value' => $args['deleted'], 'type' => PDO::PARAM_INT);
        }

        $sql = "SELECT el.*,ec.title as city
                FROM etk_locations el
    ...
    public function add($title, $slug, $city, $address) {
        ...
        $sql = "INSERT INTO etk_locations(title,slug,city_id,address,date_added) VALUES(:title,:slug,:city,:address,:date_added)";
        $command = Yii::app()->db->createCommand($sql);
